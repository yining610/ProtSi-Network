...
from config import train_to_val
...
def worker(name, episodes, q, train_dataset, valid_dataset, whole_dataset, config, gpu_id):
    ...
    for episode in range(episodes):
        ...
        new_task, new_query, new_label = train_dataset.get_mini_dataset(shots=config['training_support_shots'], 
        ...
        if episode % train_to_val==0:
            ...
            new_val_task, new_val_query , new_val_label = valid_dataset.get_mini_dataset(shots=config['training_support_shots'], 
                                                                                                             num_classes=config['training_classes'], 
                                                                                                             split=True, 
                                                                                                             query_shots=config['training_query_shots'])
