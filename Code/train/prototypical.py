...
class Prototypical(tf.keras.layers.Layer):
    ...
    def call(self, support, query, unlabel):
        ...
        if self.average: 
            prototypes_label = tf.math.reduce_mean(prototypes_label, axis=0) # average all prototypes_label over batche
        ...
        # log_p_y = tf.reshape(log_p_y, [self.batch_size, self.num_classes, self.n_query, -1])

        # # sofmax of unlabel distances (unsupervised)
