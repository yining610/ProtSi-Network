...
def read_dataset(example):
    ...
    feature_map = {'tasks': tf.io.FixedLenFeature((), tf.string),
                   ...
                   'query_labels': tf.io.FixedLenFeature((3), tf.float32),  # training_query_shots*num_classes
    ...
    return (tasks, tasks_query, paraphrase_data), (query_labels)
...
def read_model(example):
    ...
    model_answer = tf.reshape(model_answer, [max_length, embedding_size])
    return model_answer